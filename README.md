...
- **Generates a natural-language verification question per flagged claim** that opens a Google search in one click — Google's question-form ranking surfaces journalism, fact-checks, and explainers well for "is this true?" queries.
...
flowchart LR
    ...
    Ex -->|claim + verbatim<br/>+ google question<br/>+ academic search query| V{"2 · Timestamp validator<br/>seed-anchored fuzzy match"}
...
- **Two query variants per claim, generated in the same extraction pass — one natural-language question, one academic-keyword string.** The natural-language question powers the Google verify button (Google's question-form ranking surfaces journalism, fact-checks, and explainers). The academic-keyword string feeds the research lens's OpenAlex retrieval. Both share the prompt, costing zero extra API calls. A standalone "Scholar" search link was removed once the research lens shipped — it offered nothing the research lens doesn't do better with judgment and verbatim-quote evidence.
